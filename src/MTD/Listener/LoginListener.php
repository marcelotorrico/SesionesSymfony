...
class LoginListener
{
    ...
    public function onKernelResponse(FilterResponseEvent $event)
    {
        ...
        if (in_array('ROLE_ADMIN', $rolesTab) || in_array('ROLE_SUPER_ADMIN', $rolesTab)) {
            $route = $this->router->generate('mtd_registro');
        } elseif (in_array('ROLE_USER', $rolesTab)) {
            $route = $this->router->generate('mtd_estudiante_index');
        } else {
            $route = $this->router->generate('mtd_registro_login');
        }
